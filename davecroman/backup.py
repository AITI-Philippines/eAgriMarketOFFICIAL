...
form = """
    USER SIGNUP
    <form method="post">
        Username: <input type="text" name="username" value="%(username)s"></input> <label style="color:FF0000"> <label> %(username_error)s  </label> <br>
        Password: <input type="password" name="password" value=""></input> <label style="color:FF0000"> <label> %(password_error)s </label> <br>
        Verify Password: <input type="password" name="verify" value=""></input> <label style="color:FF0000"> <label> %(verify_error)s </label> <br>
        E-mail: <input type="text" name="email" value="%(email)s"></input> <label style="color:FF0000"> <label> %(email_error)s </label> <br>
        <input type="submit">
    </form>
"""
...
def encodeCharToROT13(letter=''):
    if letter.isalpha():
        capital = letter.isupper()
        letter = letter.lower()
        ascValue = ord(letter) + 13
        if ascValue > 122:
            ascValue %= 122
            ascValue += 96
        if capital:
            return chr(ascValue).upper()
        else:
            return chr(ascValue).lower()
    else:
        return letter

def encodeStringToROT13(str=''):
    newStr=''
    for i in range(0,len(str)):
        newStr += encodeCharToROT13(str[i])
    return newStr
...
def is_verify_valid(password, verify):
